...
def make_rng_pack(base_seed: int, num_point: int, dataset_tag: str):
    ...
    np_gen = np.random.RandomState(subseed + 1)
    np.random.seed(subseed)
    torch.manual_seed(subseed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(subseed)
        torch.cuda.manual_seed_all(subseed)
    return dict(subseed=subseed, scipy_rs=scipy_rs, np_gen=np_gen, base_seed=base_seed)
...
class SFCPointTokenizer(nn.Module):
    ...
    def _hilbert_order(self, pts):
        ...
        for i in range(grid.shape[0]):
            dists.append(self.hilbert.distance_from_point(grid[i].tolist()))
...
class CompactPointMamba(nn.Module):
    def __init__(self, num_points, num_classes, embed_dim=16, num_blocks=2, d_state=16, d_conv=4, expand=2):
        super().__init__()
        self.tokenizer = SFCPointTokenizer(num_points=num_points, embed_dim=embed_dim, hilbert_bits=8)
        self.blocks = nn.ModuleList(
            [MambaBlock(d_model=embed_dim, d_state=d_state, d_conv=d_conv, expand=expand) for _ in range(num_blocks)]
        )
    ...
    def forward(self, x):
        ...
        h = self.head_norm(h)
        g = h.mean(dim=1)
        return self.head_fc(g)

@torch.no_grad()
def _eval_val(model, x, y, device, criterion, batch_size):
    model.eval()
    total_loss = 0.0
    total = 0
    correct = 0
    ...
    return float(val_loss), float(val_acc)
...
def train_compact_pointmamba(
    ...
    for epoch in range(epochs):
        ...
        n = xs_aug.shape[0]
        for i in range(0, n, batch_size):
            batch_x = torch.from_numpy(xs_aug[i:i+batch_size]).to(device)
            batch_y = torch.from_numpy(ys[i:i+batch_size]).to(device)
            ...
            bs = int(batch_y.numel())
            sum_loss += float(loss.item()) * bs
            total += bs

        train_loss = sum_loss / max(total, 1)
        val_loss, val_acc = _eval_val(model, val_x, val_y, device, criterion, batch_size)
    ...
    with torch.no_grad():
        n = test_x.shape[0]
        for i in range(0, n, batch_size):
            xb = torch.from_numpy(test_x[i:i+batch_size]).to(device)
            yb = torch.from_numpy(test_y[i:i+batch_size]).to(device)
            logits = model(xb)
            pred = logits.argmax(dim=1)
    ...
    return best_epoch, best_val_acc, float(overall)
...
def main():
    ...
    best_epoch, best_val, test_acc = train_compact_pointmamba(
        dataset,
        batch_size=35,
        learning_rate=lr,
        epochs=epochs,
