...
class MyNN(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(features=24)(x)
        x = nn.tanh(x)
        x = nn.Dense(features=num_classes)(x)  # num_classes는 전역에서 설정
        return x
...
def train(gate_type, minibatch_size, Theta, epochs, key, init_scale,
    ...
    if gate_type == "u2":
        ...
        # expval length와 무관하게 기존 더미 입력 유지
        ...
        for epoch in range(epochs):
            ...
            # ----- Train -----
            current_train_x = train_dataset_x
            if use_augmentation:
            ...
            train_y_batched = train_dataset_y.reshape(num_batches, minibatch_size)
...
# dataset = np.load(f'modelnet40_5classes_{int(num_qubit/2)}_{num_reupload}_fps_train700_val100_test200_new.npz')
# dataset = np.load(f'scanobjectnn_5classes_{int(num_qubit/2)}_{num_reupload}_fps_train700_val100_test200_new.npz')
dataset = np.load(f'shapenet_5classes_{int(num_qubit/2)}_{num_reupload}_fps_train700_val100_test200_new.npz')
...
num_classes = len(np.unique(train_dataset_y))
epochs = 1000
l2 = 1e-7
...
def result(gate_type, test_learning_rate, num_blocks_reupload, init_scale, use_augmentation):
    train(gate_type, 35, Theta, epochs, key_r, init_scale,
