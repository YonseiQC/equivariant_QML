...
class MyNN(nn.Module):
    @nn.compact
    def __call__(self, x):
        x = jnp.expand_dims(x, axis=-1).reshape(x.shape[0], 10, 2)
...
dataset_tag = 'SUO'  ; dataset = np.load(f'SUO_3classes_{int(num_qubit/2)}_{num_reupload}_fps_train700_val100_test200_new.npz')
