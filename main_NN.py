...
def train(gate_type, dataset, minibatch_size, Theta, epochs, key, **adam_opt):
    ...
    if gate_type == "u2":
        ...
        params_q = init_u2 * jax.random.uniform(key, (2 * num_qubit * num_blocks,))
        ...
        for epoch in range(epochs):
            for i in range(batch_size // minibatch_size):
                ...
                # logits = NN_circuit(expval_ham, params)
                # print(loss, jnp.mean((logits > 0) == test_dataset_y))
            expval_ham = (jnp.array(u2_circuit(params, test_dataset_x))).T
            logits = NN_circuit(expval_ham, params)
        ...
        params_q = init_u3 * pnp.random.rand(4 * num_qubit * num_blocks)
        ...
        for epoch in range(epochs):
            for i in range(batch_size // minibatch_size):
                ...
                updates, opt_state = solver.update(grad, opt_state, params)
                params = optax.apply_updates(params, updates)
                #print(loss, jnp.mean((u3_circuit(params, test_dataset_x) > 0) == test_dataset_y))
...
train("u2", dataset, 16, Theta, epochs, key_r, learning_rate = 0.01)
