...
def train(minibatch_size, Theta, epochs, num_blocks_reupload, num_points, use_augmentation, sigma, num_classes, l2, learning_rate):
    ...
    params_q = init_u2 * jax.random.uniform(qkey, (repeat, num_points - 1, 2), dtype=jnp.float64)
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batches):
            ...
            q_np = np.asarray(np.array(params["q"]), dtype=np.float64, order="C")

            values, grads = circ.value_and_grad(x_scaled, q_np)
            values = jnp.asarray(values)
            grads = jnp.asarray(grads)

            feats_flat = values.reshape(minibatch_size, -1)

            (loss_val, (grad_c, grad_feats_flat)) = jax.value_and_grad(loss_from_feats, argnums=(0, 1))(params["c"], feats_flat, jnp.asarray(y_batch))
            grad_feats = grad_feats_flat.reshape(minibatch_size, 2, H)

            grad_q = jnp.tensordot(grad_feats, grads, axes=([0, 1, 2], [0, 1, 2]))

            if l2 != 0:
                loss_val = loss_val + l2 * (jnp.sum(jnp.square(params["q"])) + jnp.sum(jnp.array([jnp.sum(jnp.square(p)) for p in jax.tree.leaves(params["c"])])))
                grad_q = grad_q + 2 * l2 * params["q"]
                grad_c = jax.tree.map(lambda p: p + 2 * l2 * p, grad_c)

            grad = {"q": grad_q, "c": grad_c}
        ...
        val_logits = logits_from_feats(params["c"], val_feats_flat)
    ...
    q_np = np.asarray(np.array(params_best["q"]), dtype=np.float64, order="C")
    test_values = jnp.asarray(circ.forward(test_x_scaled, q_np))
    test_feats_flat = test_values.reshape(test_dataset_x.shape[0], -1)

    test_logits = logits_from_feats(params_best["c"], test_feats_flat)
