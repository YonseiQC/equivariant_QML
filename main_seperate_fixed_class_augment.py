...
def train(gate_type, dataset, minibatch_size, epochs_q, epochs_c, key, init_scale, num_blocks_reupload, num_blocks_circuit, num_classes, **adam_opt):
    ...
    print(get_Theta(train_dataset_x))
    ...
    # test_dataset_x = test_dataset_x.reshape(point_class_test * num_classes, num_reupload, -1, 3)
    ...
    if gate_type == "u2":
        ...
        params_c = MyNN(num_classes).init(key, dummy_input)
        ...
        def loss_fn_q_only(params_q, mini_batch_x, mini_batch_y, l2):
            ...
            logits = NN_circuit(expval_ham, params_opt_q, num_classes)
        ...
        solver_q = optax.adam(**adam_opt)
        opt_state_q = solver_q.init(params["q"])
        dummy_input = jnp.ones((1, math.comb(num_qubit, 2)))  
        params["c"] = MyNN(num_classes).init(key, dummy_input)
        solver_c = optax.adam(**adam_opt)
        opt_state_c = solver_c.init(params["c"])

        def Quantum_parameter_opt(train_dataset_x, train_dataset_y, test_dataset_x, test_dataset_y, batch_size, minibatch_size, params, opt_state_q, key):
                ...
                for j in range(i + 1, 10):
                    ...
                    for epoch in range(epochs_q):
                        ...
                        for l in range((batch_size // minibatch_size) // 5):
                            ...
                            perm = jax.random.permutation(subkey, train_dataset_x_q[l].shape[-2])
                            train_dataset_x_q = train_dataset_x_q.at[l].set(train_dataset_x_q[l][:, :, perm, :])
                        ...
                        succed = jnp.mean(predictions == test_dataset_y_q)

                        class_i_mask = (test_dataset_y_q == i)  
                        class_j_mask = (test_dataset_y_q == j) 

                        class_i_acc = jnp.mean(predictions[class_i_mask] == i) if jnp.any(class_i_mask) else 0.0
                        class_j_acc = jnp.mean(predictions[class_j_mask] == j) if jnp.any(class_j_mask) else 0.0
                        
                        print(test_loss)
                        print(f"Quantum_{epoch}, Overall: {succed:.4f}")
                        print(f"Class {i}: {class_i_acc:.4f}")
                        print(f"Class {j}: {class_j_acc:.4f}")
                        print("-" * 30)
            ...
            return params, opt_state_q


        def Classical_parameter_opt(train_dataset_x, train_dataset_y, test_dataset_x, test_dataset_y, batch_size, minibatch_size, params, opt_state_c):
            ...
            for epoch in range(epochs_c):
                ...
                for i in range(batch_size // minibatch_size):
                    ...
                    # loss, grad = jax.value_and_grad(loss_fn, argnums=0)(params, train_dataset_x[i], train_dataset_y[i])
                    updates, opt_state_c = solver_c.update(grad, opt_state_c, params["c"])
            ...
            return params, opt_state_c


        for cycle in range(2):
            params, opt_state_q = Quantum_parameter_opt(train_dataset_x, train_dataset_y, test_dataset_x, test_dataset_y, batch_size, minibatch_size, params, opt_state_q, key)
            params, opt_state_c = Classical_parameter_opt(train_dataset_x, train_dataset_y, test_dataset_x, test_dataset_y, batch_size, minibatch_size, params, opt_state_c)
        ...
        final_cm, final_class_acc, final_overall_acc = calculate_final_metrics(test_dataset_y, final_predictions, num_classes)
...
## twirling gate 만들어보기 -> initial state problem 해결 안됨
## 뒤에 neural network -> set attention?
## 위의 코드 qugmentation 추가해서 해보기
## data 전처리하기
## 논문 배경지식 조금씩 써두기
## u3 고치기