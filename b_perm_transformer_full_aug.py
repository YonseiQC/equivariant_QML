...
class MyNN(nn.Module):
    ...
    def __call__(self, x):
        ...
        # 모든 pooling 결합 
        x = jnp.concatenate([
            mean_pool, max_pool, min_pool, attention_pool, sum_pool, std_pool, var_pool
            # attention_pool_2,
...
# # memory : qubits = 8, gate_type = u2, test_learning_rate = 0.002, num_blocks_reupload = 12, init_scale= 0.02, num_reupload = 1, Theta = 1.7, l2 = 1e-6, layer_1 = 64, layer_2 = 64, attention = 32-> modelnet(5classes) -> 74.5%(attention 추가) 
# # memory : qubits = 8, gate_type = u2, test_learning_rate = 0.002, num_blocks_reupload = 12, init_scale= 0.02, num_reupload = 1, Theta = 1.7, l2 = 1e-6, layer_1 = 64, layer_2 = 64, attention = 32-> modelnet(5classes) -> 74.5%(attention 추가) 

# 📝 Data Augmentation 변경사항:
# ✅ scipy special_ortho_group.rvs(3) - 수학적으로 완벽한 3D 회전
# ✅ 기존 Z축 회전에서 완전한 3D 회전으로 업그레이드
# ✅ JAX와 호환되도록 numpy 배열을 jnp.array로 변환