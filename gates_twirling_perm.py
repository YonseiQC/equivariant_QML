...
class Spin_twirling(qml.operation.Operation):
    ...
    def compute_decomposition(theta0, theta1, num_perm, wires):
        ...
        return [
            qml.QubitUnitary(U_1.conj().T, wires=range(num_qubit)),
            qml.DiagonalQubitUnitary(jnp.exp(1j * theta0 * normalized_eigenvals_1), wires=range(num_qubit)),
            qml.QubitUnitary(U_1, wires=range(num_qubit)),
            qml.QubitUnitary(U_2.conj().T, wires=range(num_qubit)),
            qml.DiagonalQubitUnitary(jnp.exp(1j * theta1 * normalized_eigenvals_2), wires=range(num_qubit)),
            qml.QubitUnitary(U_2, wires=range(num_qubit))
        ]