...
from gates_fast_rot_only import (
    Spin_twirling_rot_only,
    create_singlet,
    num_rot_only_params,
    rot_only_generator_count,
)
...
def create_twirling_circuit(num_qubit_, num_blocks_reupload, Theta_, H):
    def twirling_circuit(params, data_pt):
        ...
        for _ in range(num_blocks_reupload):
            for cycle_length in range(2, int(num_qubit_ / 2) + 1):
                count = rot_only_generator_count(num_qubit_, cycle_length)
                Spin_twirling_rot_only(
                    params["q"][offset : offset + count],
                    cycle_length,
                    wires=range(num_qubit_),
                )
                offset += count
...
def train(
    *,
    ...
    num_q_params = num_rot_only_params(num_qubit_, num_blocks_reupload)
    init_u2 = init_scale * math.pi / num_q_params
    qkey = jax.random.PRNGKey(make_subseed(global_subseed, "init_q"))
    params_q = init_u2 * jax.random.uniform(qkey, (num_q_params,))
