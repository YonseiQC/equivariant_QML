...
_OUT_DIR = Path(__file__).resolve().parent / "PermMatrix"
...
def generate_permutation_matrices(num_qubit):
    ...
    for num_perm in range(2, max_perm + 1):
        ...
        np.save(_OUT_DIR / f"perm_matrix_{num_qubit}_{num_perm}_plus_normalized.npy", plus_matrix)
        np.save(_OUT_DIR / f"perm_matrix_{num_qubit}_{num_perm}_minus_normalized.npy", minus_matrix)

        print(
            f"Saved perm_matrix_{num_qubit}_{num_perm}_plus_normalized.npy and perm_matrix_{num_qubit}_{num_perm}_minus_normalized.npy"
        )
...
def main():
    ...
    generate_permutation_matrices(args.num_qubit)
