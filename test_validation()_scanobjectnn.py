...
class MyNN(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(features=24)(x)
        x = nn.tanh(x)
        x = nn.Dense(features=3)(x)  # 로직/차원 불변
        return x
...
def train(gate_type, minibatch_size, Theta, epochs, key, init_scale,
    ...
    if gate_type == "u2":
        ...
        for epoch in range(epochs):
            ...
            print(f"  Train Loss      : {float(epoch_train_loss):.6f}")
            ...
            print(f"  Val   Accuracy  : {float(val_acc)*100:.2f}%")
            print(f"  Grad Norms      : Q={float(avg_q):.2e} | C={float(avg_c):.2e} | Total={float(avg_tot):.2e}")
            print("-" * 60)
        ...
        print("\n=== Best on Validation ===")
        print(f" Best Val Accuracy : {float(best_val_acc)*100:.2f}% @ epoch {best_epoch}")
        print(" Evaluating on TEST set at best-val checkpoint ...")
        ...
        print(f" Test Overall Accuracy : {float(final_overall_acc)*100:.2f}%")
        print(" Class-wise Accuracy   : " + " | ".join([f"C{i}:{acc*100:.2f}%" for i, acc in enumerate(final_class_acc)]))
        # 필요시 Confusion Matrix/Report는 주석 해제
...
for _base_seed in [1557, 831, 121, 2023, 2024, 2025, 2026]:
        ...
        dataset_tag = 'scanobjectnn'
        dataset = np.load(f'scanobjectnn_3classes_{int(num_qubit/2)}_{num_reupload}_fps_train700_val100_test200_new.npz')
        # 상단 정보 출력(포맷 통일)
        _Theta = get_Theta(dataset)
        print("\n" + "=" * 60)
        print(f"[DATASET] tag={dataset_tag} | points={int(num_qubit/2)} | reupload={num_reupload}")
        print(f"[SEED   ] {_base_seed}")
        try:
            print(f"[Theta  ] {float(_Theta):.6f}")
        except Exception:
        ...
        def result(gate_type, test_learning_rate, num_blocks_reupload, init_scale, use_augmentation):
            ...
            # 러닝 설정 요약
            print(f"[RUN    ] gate={gate_type} | blocks={num_blocks_reupload} | epochs={epochs} | "
                f"lr={test_learning_rate} | l2={l2} | minibatch=35")
...
print("\n" + "#" * 60)
try:
    print("[RESULTS]", [float(x) for x in results])
except Exception:
