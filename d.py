...
jax.config.update("jax_platform_name", "gpu")  # TPU → GPU 변경

key = jax.random.PRNGKey(1557)
...
def encode(point, num_qubit):  
    ...
    for i in range(int(num_qubit / 2)):
        ...
        ], dtype=jnp.complex64).transpose(2,0,1)
...
def train(gate_type, dataset, minibatch_size, Theta, epochs, key, init_scale, num_blocks_reupload, num_qubit, num_reupload, **adam_opt):  
    ...
    if gate_type == "u2":
        ...
        params_q = jnp.array(init_u2 * jax.random.uniform(key, (2 * (num_blocks_reupload * num_reupload),)), dtype=jnp.float32)

        dummy_input = jnp.ones((1, math.comb(int(num_qubit / 2), 2)), dtype=jnp.float32)  
        ...
        @jax.checkpoint
        def forward_pass(params, mini_batch_x):
            return (jnp.array(twirling_circuit(params, mini_batch_x))).T

        @jax.checkpoint  
        def nn_forward(expval_ham, params):
            return NN_circuit(expval_ham, params)

        def loss_fn(params, mini_batch_x, mini_batch_y, l2):
            expval_ham = forward_pass(params, mini_batch_x)
            logits = nn_forward(expval_ham, params)
        ...
        # 성능 테스트 및 예상 시간 계산
        print("Testing single batch performance...")
        import time
        start_time = time.time()
        test_loss = loss_fn(params, train_dataset_x[0], train_dataset_y[0], l2)
        single_batch_time = time.time() - start_time
        total_batches = batch_size // minibatch_size
        print(f"Single batch time: {single_batch_time:.2f}s")
        print(f"Total batches per epoch: {total_batches}")
        print(f"Estimated epoch time: {single_batch_time * total_batches / 60:.1f} minutes")
        ...
        for epoch in range(epochs):
            ...
            for i in range(total_batches):
                # 진행상황 출력
                if i % 50 == 0:
                    print(f"Epoch {epoch}, Batch {i}/{total_batches} ({i/total_batches*100:.1f}%)")
            ...
            expval_ham = forward_pass(params, test_dataset_x)
            logits = nn_forward(expval_ham, params)
            ...
            for j, acc in enumerate(class_accuracies):
                print(f"Class {j} Accuracy: {acc:.4f}")
            print("-" * 40)
            
            # 메모리 해제
            del expval_ham, logits, predictions
        
        max_success = max(succed_lst)
        print(f"\n=== FINAL RESULTS ===")
        print(f"Maximum Test Accuracy: {max_success:.4f}")
        
        final_expval_ham = forward_pass(params, test_dataset_x)
        final_logits = nn_forward(final_expval_ham, params)
...
def result(gate_type, test_learning_rate, num_blocks_reupload, init_scale):  
    ...
    train(gate_type, dataset, 1, Theta, epochs, key_r, init_scale, num_blocks_reupload, num_qubit, num_reupload, learning_rate = test_learning_rate) 
